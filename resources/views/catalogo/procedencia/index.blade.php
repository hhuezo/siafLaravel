...
@section('content')
                        ...
                        <table id="datatable-basic" class="table table-striped text-nowrap w-100">
                            ...
                            <tbody>
                                @foreach ($procedencias as $item)
                                    ...
                                    @include('catalogo.procedencia.edit')
                                    @include('catalogo.procedencia.delete')
                                @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title" id="exampleModalLgLabel">Crear Procedencia</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" action="{{ route('procedencia.store') }}">
                    @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            expandMenuAndHighlightOption('catalogoMenu', 'procedenciaOption');
